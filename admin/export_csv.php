...
if ($tipo === 'clientes') {
    ...
    $stmt = $pdo->prepare("SELECT id, nome_razao, cpf_cnpj, tipo_pessoa, email, telefone, celular, cidade, estado, ativo, criado_em FROM clientes WHERE admin_id = ? ORDER BY nome_razao");
    $stmt->execute([$adminIdE]);
    $stmt = $stmt->fetchAll();
    foreach ($stmt as $r) {
        fputcsv($output, [
