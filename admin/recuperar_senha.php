...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (empty($email)) {
        ...
        $pdo = getConnection();
        $stmt = $pdo->prepare("SELECT id, nome, email FROM administradores WHERE email = ? AND ativo = 1");
        $stmt->execute([$email]);
        $admin = $stmt->fetch();
        ...
        if ($admin) {
            ...
            $upd = $pdo->prepare("UPDATE administradores SET token_recuperacao = ?, token_recuperacao_expira = DATE_ADD(NOW(), INTERVAL 1 HOUR) WHERE id = ?");
            $upd->execute([$hash, $admin['id']]);

            $scheme = (!empty($_SERVER['HTTPS']) && $_SERVER['HTTPS'] !== 'off') ? 'https' : 'http';
            $link = $scheme . '://' . ($_SERVER['HTTP_HOST'] ?? 'localhost') . '/cobranca/admin/redefinir_senha.php?token=' . $token;

            if (enviarEmailRecuperacaoSenha($admin['email'], $admin['nome'], $link, 'admin')) {
