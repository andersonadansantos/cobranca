...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (strlen($senha) < 6) {
        ...
        $stmt = $pdo->prepare("SELECT id FROM administradores WHERE token_recuperacao = ? AND token_recuperacao_expira > NOW() AND ativo = 1");
        $stmt->execute([$hash]);
        $admin = $stmt->fetch();

        if ($admin) {
            $newHash = password_hash($senha, PASSWORD_BCRYPT);
            $upd = $pdo->prepare("UPDATE administradores SET senha = ?, token_recuperacao = NULL, token_recuperacao_expira = NULL WHERE id = ?");
    ...
    <div class="login-page">
        <div class="login-split">
            <div class="login-left">
                ...
                <p>Defina uma nova senha para acessar seu painel <?= htmlspecialchars($nomeSistema) ?>.</p>
            </div>
                ...
                <div class="login-form">
                    ...
                    <?php if ($mensagem): ?>
                        ...
                        <div class="text-center mt-3">
                            <a href="/cobranca/admin/login.php" class="btn btn-primary w-100">
